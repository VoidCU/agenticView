 ...
 */
export function planOfficeWithSpaces(spaces, agents) {
    const workers = agents.filter((a) => a.role === "worker").sort(byCreation);
    const byId = new Map(spaces.map((s) => [s.id, s]));
    const taken = new Set();
    const placements = {};
 ...
 */
export function hexesForRing(ring) {
    if (ring === 1) {
        return [
 ...
 */
export function currentOfficeRings(rooms) {
    if (rooms.length === 0)
        return 0;
 ...
 */
export function nextAddRoomHex(rooms) {
...
export function nextRoomId(kind, rooms) {
    if (kind === "pod") {
    ...
    if (count === 0)
        return kind; // 'meeting' or 'lounge'
    return `${kind}-${count + 1}`;
}
/** Builds a Space[] from an explicit room list (always prepends the Manager's Office at ring 0). */
export function buildSpacesFromExplicit(rooms) {
    const mk = (id, name, kind, q, r) => ({
        id,
        name,
        kind,
        q,
