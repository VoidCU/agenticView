...
let office: Instance | undefined;
let current: { runId: string; office: Instance } | undefined;
...
async function post<T>(inst: Instance, path: string, body: unknown, signal?: AbortSignal): Promise<T> {
  ...
  if (res.status === 401) throw new Error("The office rejected this worker's token (it was probably restarted). Call agenticview_next_task again.");
  if (!res.ok) throw new Error(`Office returned HTTP ${res.status}`);
...
async function nextTask(args: { wait_seconds?: number; project?: string }): Promise<ToolText> {
  if (current) {
  ...
  while (Date.now() < deadline) {
    ...
    try {
      const { task } = await post<{ task: SessionTask | null }>(office, "/api/worker/claim", { waitMs }, AbortSignal.timeout(waitMs + 10_000));
      if (task) {
...
function ackText(ack: Ack, okText: string): ToolText {
  ...
  if (ack.cancelled) current = undefined;
...
export async function main(): Promise<void> {
  ...
  server.registerTool(
    ...
    {
      description:
        "Call one of the current task's office tools (listed under 'Office tools' in the task), e.g. delegate/assign_task/await_tasks for a Manager, or report/ask tools for workers.",
