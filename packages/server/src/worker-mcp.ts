...
interface State {
  office?: Instance;
  /** Runs this session holds (claimed, not completed), by run id. */
  runs: Map<string, HeldRun>;
  /** Claude Code session id (from the skill's ), else a per-process id. */
  sessionId: string;
  model?: string;
  agent?: string;
}
const state: State = { sessionId: processWorkerId, runs: new Map() };
...
export function _testInjectRun(runId: string, office: Instance, agent: string): void {
  state.runs.set(runId, { office, agent, subagent: null });
  state.office = office;
}

/** True when a and b describe the same running office process. */
function isSameInstance(a: Instance, b: Instance): boolean {
 ...
 */
export function dropRunsForOldOffice(newOffice: Instance): string[] {
...
function recentWorkLines(t: SessionTask): string[] {
  const work = t.recentWork ?? [];
  if (work.length === 0) return ["(none yet: this is the agent's first task here)"];
  return work.map((w) => {
...
export function formatDispatch(t: SessionTask, i: number, n: number): string {
...
function cancelledText(ids: string[]): string {
  return ids
    .map((id) => {
...
export async function nextTask(args: NextTaskArgs): Promise<ToolText> {
  ...
  if (!discovered) return reply(NO_OFFICE, true);
  if (state.office && !isSameInstance(state.office, discovered)) dropRunsForOldOffice(discovered);
  ...
  for (;;) {
    ...
    try {
      const res = await post<ClaimReply>(state.office, "/api/worker/claim", { waitMs, session, holding: heldRuns(), ...(max !== undefined ? { max } : {}) }, AbortSignal.timeout(waitMs + 10_000));
      const cancelled = res.cancelled ?? [];
      const notice = cancelled.length ? cancelledText(cancelled) : "";
      for (const id of cancelled) state.runs.delete(id);
      // Finished elsewhere (or unknown after an office restart): nothing to stop, just forget them.
      for (const id of res.gone ?? []) state.runs.delete(id);
      const tasks = res.tasks ?? (res.task ? [res.task] : []);
      for (const t of tasks) state.runs.set(t.runId, { office: state.office, agent: t.agent.name, subagent: t.subagent ?? null });
      ...
      if (notice) return reply(`${notice}\n\n${status}`);
      // max_tasks 0: only a check for cancellations, never a wait.
      if (max === 0) return reply(`No cancellations. ${status}`);
    } catch {
      ...
      if (!fresh) return reply(NO_OFFICE, true);
      if (!state.office || !isSameInstance(state.office, fresh)) dropRunsForOldOffice(fresh);
      state.office = fresh;
    }
...
function ackText(ack: Ack, okText: string, runId: string): ToolText {
  if (ack.ok) return reply(ack.result ?? okText);
  if (ack.cancelled) state.runs.delete(runId);
 ...
 */
async function resolveRun(runId: unknown): Promise<{ runId: string; office: Instance } | ToolText> {
  const id = typeof runId === "string" ? runId.trim() : "";
  if (id) {
    const held = state.runs.get(id);
    if (held) return { runId: id, office: held.office };
    const office = state.office ?? (await discoverOffice(startDir()));
    if (!office) return reply(NO_OFFICE, true);
    return { runId: id, office };
  }
  const ids = heldRuns();
  if (ids.length === 1) return { runId: ids[0]!, office: state.runs.get(ids[0]!)!.office };
  if (ids.length === 0) return reply("No active AgenticView task. Call agenticview_next_task first (or pass run_id).", true);
 ...
 */
async function checkOfficeRestart(runId: string): Promise<ToolText | undefined> {
  const fresh = await discoverOffice(startDir());
  if (!fresh) return undefined; // office is gone entirely; caller reports the original error
  if (state.office && isSameInstance(state.office, fresh)) return undefined; // same instance; not a restart
  ...
  return reply(
    `Office restarted: run ${runId} is no longer active on the new instance.${dropped.length > 1 ? ` ${dropped.length - 1} other stale run(s) also dropped.` : ""} Call agenticview_next_task to pick up new work from the restarted office.`,
    !affected, // isError only when we didn't already remove the run (it was already gone)
  );
}

export async function report(args: RunArgs & { text?: string; events?: z.infer<typeof EventSchema>[] }): Promise<ToolText> {
  identify(args);
  ...
  try {
    return ackText(await post<Ack>(cur.office, `/api/worker/${cur.runId}/report`, { events, subagentId: args.subagent_id }), "Reported.", cur.runId);
  } catch (e) {
    return (await checkOfficeRestart(cur.runId)) ?? reply(`ERROR: ${(e as Error).message}`, true);
  }
}

export async function complete(args: RunArgs & { result?: string; error?: string }): Promise<ToolText> {
  identify(args);
  const cur = await resolveRun(args.run_id);
  if ("content" in cur) return cur;
  try {
    const ack = await post<Ack>(cur.office, `/api/worker/${cur.runId}/complete`, { text: args.result, error: args.error, subagentId: args.subagent_id });
...
export async function bridge(args: RunArgs & { tool: string; args?: Record<string, unknown> }): Promise<ToolText> {
  identify(args);
  const cur = await resolveRun(args.run_id);
  if ("content" in cur) return cur;
  try {
    return ackText(await post<Ack>(cur.office, `/api/worker/${cur.runId}/bridge`, { name: args.tool, args: args.args ?? {} }), "", cur.runId);
  } catch (e) {
    return (await checkOfficeRestart(cur.runId)) ?? reply(`ERROR: ${(e as Error).message}`, true);
  }
}
...
export async function main(): Promise<void> {
  ...
  server.registerTool(
    ...
    {
      description:
        "Stream progress for a task to the office feed. Pass run_id, a short `text` update and/or structured `events` (tool_start/tool_end with name and summary, file_changed with path and kind).",
      inputSchema: {
  ...
  server.registerTool(
    ...
    {
      description:
        "Call one of a task's office tools (listed under 'Office tools' in the task), e.g. assign_task/await_tasks for a Manager, or take_screenshot for workers. Pass run_id. await_tasks returns after about 4 minutes with 'stillRunning' ids if tasks are not done; call it again with those ids.",
      inputSchema: {
        run_id: z.string().optional().describe(RUN_ID_HINT),
