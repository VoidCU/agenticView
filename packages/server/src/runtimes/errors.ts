 ...
 */
export function extractCliError(raw: string): string {
  ...
  try {
    ...
    if (parsed && typeof parsed === "object") {
      if (typeof parsed.message === "string") return extractCliError(parsed.message);
      if (parsed.error && typeof parsed.error === "object") {
        if (typeof parsed.error.message === "string") return extractCliError(parsed.error.message);
      }
    }
  } catch {
  ...
  const isNoise = (line: string) => {
    if (/^reading (?:prompt|additional input) from stdin\.\.\./i.test(line)) return true;
    if (/^using model\b/i.test(line)) return true;
    if (/^failed to parse item:\s*reading/i.test(line)) return true;
    return false;
  };
  ...
  const prominent = filtered.filter((l) =>
    /^ERROR:/i.test(l) ||
 ...
 * - ISO 8601 strings
 */
export function parseResetAt(errorText: string, now: Date = new Date()): string | undefined {
  ...
  const timeMatch =
    errorText.match(/(?:try again|resets?|retry)\s+(?:at\s+)?(\d{1,2}):(\d{2})(?::(\d{2}))?\s*(AM|PM)?/i) ||
  ...
  if (relMatch) {
    ...
    let ms = 0;
    if (unit.startsWith("s")) ms = count * 1000;
    else if (unit.startsWith("m")) ms = count * 60 * 1000;
    else if (unit.startsWith("h")) ms = count * 3600 * 1000;
    else if (unit.startsWith("d")) ms = count * 86400 * 1000;
 ...
 */
export function isLimitActive(limit?: LimitInfo, now: Date = new Date()): boolean {
  if (!limit || !limit.limited) return false;
