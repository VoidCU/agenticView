...
export class GeminiRuntime implements Runtime {
  ...
  private async installBridgeSettings(req: RunRequest): Promise<() => Promise<void>> {
    ...
    if (original) {
      try {
        settings = JSON.parse(original) as Record<string, unknown>;
      } catch {
        settings = {};
      }
    }
    ...
    return async () => {
      if (original !== undefined) await writeFile(file, original, "utf8");
        ...
        if (!dirExisted) await rmdir(dir).catch(() => undefined);
      }
    };
  }
