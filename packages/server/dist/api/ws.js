...
async function handle(msg, world, opts, send) {
    ...
    switch (msg.type) {
        ...
        case "session.rename": {
            ...
            if (!rt || !(await rt.rename(msg.id, msg.name)))
            ...
            for (const a of await registry.list()) {
                if (a.session?.id === msg.id)
                    bus.emit({ type: "agent.updated", agent: await registry.update(a.id, { session: { id: msg.id, name: msg.name.trim() } }) });
            }
            return;
        }
        ...
        case "session.forget": {
            const rt = world.sessionRuntime;
            if (!rt)
                throw new Error("The Claude Code session provider is not configured");
            await rt.forget(msg.id);
            for (const a of await registry.list()) {
                if (a.session?.id === msg.id)
                    bus.emit({ type: "agent.updated", agent: await registry.update(a.id, { session: null }) });
            }
            return;
        }
        case "project.open": {
