...
for (const t of tools) {
    server.registerTool(t.name, { description: t.description, inputSchema: jsonSchemaToZodShape(t.inputSchema) }, async (args) => {
        const res = (await (await fetch(`${url}/bridge/${runId}/call`, { method: "POST", headers, body: JSON.stringify({ name: t.name, args }) })).json());
