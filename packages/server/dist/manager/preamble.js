...
export function buildRosterPreamble(world, agents, tasks) {
    const open = tasks.filter((t) => !isTerminal(t.status));
    const runningBy = new Map();
    for (const t of open)
        if (t.status === "running" || t.status === "waiting")
            runningBy.set(t.assigneeId, t.id);
    const lines = [`# AgenticView ${world.kind} world: ${world.name}`];
    if (world.projectPath)
        lines.push(`Project path: ${world.projectPath}`);
    else
        lines.push(`Known projects: ${world.knownProjects.map((p) => `${p.name} (${p.path})`).join(", ") || "none"}`);
    lines.push("", "## Roster");
    ...
    if (workers.length === 0)
        lines.push("- (no workers yet; use create_agent)");
