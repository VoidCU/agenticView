...
const state = { sessionId: processWorkerId };
...
export async function nextTask(args) {
    ...
    while (Date.now() < deadline) {
        ...
        try {
            const { task } = await post(state.office, "/api/worker/claim", { waitMs, session }, AbortSignal.timeout(waitMs + 10_000));
            if (task) {
...
export async function main() {
    ...
    server.registerTool("agenticview_bridge", {
        description: "Call one of the current task's office tools (listed under 'Office tools' in the task), e.g. delegate/assign_task/await_tasks for a Manager, or report/ask tools for workers. await_tasks returns after about 4 minutes with 'stillRunning' ids if tasks are not done; call it again with those ids.",
        inputSchema: {
