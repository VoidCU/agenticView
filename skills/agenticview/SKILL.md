...
- Agents run on the provider each agent is set to. Claude (API) agents need `ANTHROPIC_API_KEY` in the environment (the Agent SDK does not reuse the Claude Code login); on a Max/Pro plan use the "Claude Code session" provider instead and run `/agenticview-work` in a Claude Code session so it pulls those tasks; Codex agents need the `codex` CLI signed in; Antigravity agents need the Antigravity CLI (`agy`) signed in with an Antigravity account (no API key); Gemini agents need the `gemini` CLI signed in (a Gemini API key or a Google Cloud project). The office's settings panel shows which providers are available and why.
