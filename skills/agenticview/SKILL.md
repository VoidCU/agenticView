...
- If `~/.agenticview/plugin-root` is missing, ask the user to restart Claude Code once so the plugin's SessionStart hook can record where the plugin lives, or run `npx agenticview open --project "$PWD"` instead.
